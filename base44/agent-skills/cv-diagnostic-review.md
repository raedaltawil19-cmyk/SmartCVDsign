...
8. **Local labour-market conventions** — evaluate against the conventions of the labour market the CV targets, where genuinely relevant.

**Boundary:** Do not perform ATS/job-keyword scoring in a general CV review. ATS analysis requires a specific target vacancy and belongs to the Job Tailor workflow. You may still note general readability or terminology problems that affect machine readability, but do not call them an ATS match score.
...
- **How does the CV compare to a real target job, including ATS/keyword alignment?** → This is outside the general diagnostic skill and belongs to the Job Tailor workflow (`job_requirement_matching` + ATS analysis). Do not perform job-specific matching as part of a general CV diagnosis.
