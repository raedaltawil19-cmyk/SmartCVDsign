...
export default function TailorRecommendationRow({ rec, selected, sent = false, onToggle }) {
  return (
    <li>
      ...
      >
        <span className={`shrink-0 mt-0.5 w-4 h-4 rounded border grid place-items-center ${sent ? "bg-green-600 border-green-600 text-white" : selected ? "bg-[#000066] border-[#000066] text-white" : "border-slate-300 bg-white"}`}>
          {(selected || sent) && <Check className="w-3 h-3" />}
        </span>
        <span className="flex-1 min-w-0">
          <span className="flex items-center gap-1.5 mb-1">
            ...
            {sent && <span className="text-[10px] px-1.5 py-0.5 rounded-md border border-green-200 bg-green-50 text-green-700 font-medium">تم الإرسال ✓</span>}
          </span>
