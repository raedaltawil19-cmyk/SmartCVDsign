...
export default function CVReviewCard({
  ...
  onClose,
  onSendToAssistant,
  // نصوص عرض اختيارية (UX فقط) — القيم الافتراضية تحفظ سلوك M5/M6 كما هو
  heading = "مراجعة سيرتك الذاتية",
  subnote,
  hint = "التحسينات المقترحة — اختر ما يهمك، لن يُطبَّق شيء الآن.",
  sendLabel = "أرسل المحدد إلى مساعد السيرة",
  sendNote = "سيتولّى مساعد السيرة تنفيذ ما اخترته فقط.",
  emptyNote = "اختر توصية واحدة أو أكثر لتنفيذها."
}) {
  ...
  return (
    <div dir="rtl" className="no-print bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-slate-100">
        ...
        <h3 className="text-[13px] font-semibold text-slate-800">{heading}</h3>
      ...
      <div className="px-4 py-3 max-h-[38vh] overflow-y-auto">
        {subnote && (
          <p className="text-[11px] leading-relaxed text-[#000066] bg-[#000066]/5 border border-[#000066]/10 rounded-xl px-3 py-2 mb-3">{subnote}</p>
        )}
        {noImprovement ? (
      ...
      {!noImprovement && onSendToAssistant && (
        <div className="flex items-center gap-2 px-4 py-2.5 border-t border-slate-100 bg-slate-50">
          <p className="text-[11px] text-slate-500">
            {selectedIds.length > 0 ? sendNote : emptyNote}
          </p>
          <button
            type="button"
