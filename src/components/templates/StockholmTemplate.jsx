...
export default function StockholmTemplate({ data: d, editable, actions }) {
  return (
      ...
      <aside className="w-[34%] bg-slate-50 p-8 space-y-8 border-r border-slate-200">
        ...
        <div>
          ...
          <div className="space-y-2">
            ...
            {editable && <button onClick={actions.addSkill} className="no-print text-[12px] text-[#1B4FD8] flex items-center gap-1"><Plus className="w-3 h-3" />Lägg till</button>}
          </div>
        </div>
        <div>
          ...
          <div className="space-y-1.5">
            ...
            {editable && <button onClick={actions.addSprak} className="no-print text-[12px] text-[#1B4FD8] flex items-center gap-1"><Plus className="w-3 h-3" />Lägg till</button>}
          </div>
        </div>
      </aside>
      <main className="flex-1 p-10 space-y-7 bg-white">
        ...
        <section>
          ...
          <div className="space-y-5">
            ...
            {editable && <button onClick={actions.addExp} className="no-print text-[12px] text-[#1B4FD8] flex items-center gap-1"><Plus className="w-3 h-3" />Lägg till erfarenhet</button>}
          </div>
        </section>
        <section>
          ...
          <div className="space-y-4">
            ...
            {editable && <button onClick={actions.addEdu} className="no-print text-[12px] text-[#1B4FD8] flex items-center gap-1"><Plus className="w-3 h-3" />Lägg till utbildning</button>}
          </div>
        </section>
      </main>
    </div>
