...
export function formatIntentMessage(intent) {
  ...
  return [
    fromTailor
      ? "توصية صادرة عن مخصّص الطلبات (تحليل وظيفة) اختارها المستخدم صراحةً وأرسلها إلى مساعد السيرة للتنفيذ. هذا الإرسال هو تفويض تنفيذ صريح: لا تطلب تأكيداً إضافياً ولا تعُد إلى سؤال المستخدم عن صحة الشهادة أو الخبرة أو أي معلومة ضمن التوصية. الهدف هو النسخة المخصّصة المفتوحة أمامك الآن في CV_CONTEXT — لا تلمس أي سيرة أخرى. اقرأ حالتها الحالية، ثم نفّذ التوصية حرفياً بأدواتك المعتمدة."
    ...
    "نفّذ هذه التوصية وحدها، ولا تلمس شيئاً آخر. لا تسأل عن تأكيد إضافي لهذه التوصية؛ اختيارها وإرسالها من مسار application_tailor هو الموافقة التنفيذية النهائية.",
